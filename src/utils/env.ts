...
export function validateEnv(): void {
  ...
  const requiredInProd = [ENV_VARS.OPENAI_API_KEY, ENV_VARS.ANTHROPIC_API_KEY];
  ...
  if (isProd) {
    ...
    // Check all required env vars in production
    ...
    // In development, just log warnings for missing API keys
    if (!hasEnv(ENV_VARS.OPENAI_API_KEY)) {
      console.warn("OPENAI_API_KEY not set.");
    }

    if (!hasEnv(ENV_VARS.ANTHROPIC_API_KEY)) {
